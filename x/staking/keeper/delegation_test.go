...
func TestUnbondDelegation(t *testing.T) {
	...
	require.NoError(t, testutil.FundModuleAccount(app.BankKeeper, ctx, notBondedPool.GetName(), sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, startTokens))))
...
func TestUndelegateSelfDelegationBelowMinSelfDelegation(t *testing.T) {
	...
	_, err := app.StakingKeeper.Undelegate(ctx, val0AccAddr, addrVals[0], sdk.NewDecFromInt(app.StakingKeeper.TokensFromConsensusPower(ctx, 6)), sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(100)))
	require.NoError(t, err)
...
func TestUndelegateFromUnbondingValidator(t *testing.T) {
	...
	_, err := app.StakingKeeper.Undelegate(ctx, val0AccAddr, addrVals[0], sdk.NewDecFromInt(delTokens), sdk.NewCoin(sdk.DefaultBondDenom, delTokens))
	require.NoError(t, err)
	...
	_, err = app.StakingKeeper.Undelegate(ctx, addrDels[1], addrVals[0], sdk.NewDec(6), sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(6)))
	require.NoError(t, err)
...
func TestUndelegateFromUnbondedValidator(t *testing.T) {
	...
	// unbond the all self-delegation to put validator in unbonding state
	_, err := app.StakingKeeper.Undelegate(ctx, val0AccAddr, addrVals[0], sdk.NewDecFromInt(valTokens), sdk.NewCoin(sdk.DefaultBondDenom, valTokens))
	require.NoError(t, err)
	...
	_, err = app.StakingKeeper.Undelegate(ctx, addrDels[1], addrVals[0], sdk.NewDecFromInt(unbondTokens), sdk.NewCoin(sdk.DefaultBondDenom, unbondTokens))
	require.NoError(t, err)

	// unbond rest of the other delegation's shares
	remainingTokens := delTokens.Sub(unbondTokens)
	_, err = app.StakingKeeper.Undelegate(ctx, addrDels[1], addrVals[0], sdk.NewDecFromInt(remainingTokens), sdk.NewCoin(sdk.DefaultBondDenom, remainingTokens))
	require.NoError(t, err)
...
func TestUnbondingAllDelegationFromValidator(t *testing.T) {
	...
	_, err = app.StakingKeeper.Undelegate(ctx, addrDels[1], addrVals[0], sdk.NewDecFromInt(delTokens), sdk.NewCoin(sdk.DefaultBondDenom, delTokens))
	require.NoError(t, err)
...
func TestRedelegateToSameValidator(t *testing.T) {
	...
	_, err := app.StakingKeeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[0], sdk.NewDec(5), sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(5)))
	require.Error(t, err)
}
...
func TestRedelegationMaxEntries(t *testing.T) {
	...
	for i := uint32(0); i < maxEntries; i++ {
		var err error
		completionTime, err = app.StakingKeeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], sdk.NewDec(1), sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(1)))
		require.NoError(t, err)
	}

	// an additional redelegation should fail due to max entries
	_, err := app.StakingKeeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], sdk.NewDec(1), sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(1)))
	require.Error(t, err)
...
func TestRedelegateSelfDelegation(t *testing.T) {
	...
	_, err := app.StakingKeeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], sdk.NewDecFromInt(delTokens), sdk.NewCoin(sdk.DefaultBondDenom, delTokens))
	require.NoError(t, err)
...
func TestRedelegateFromUnbondingValidator(t *testing.T) {
	...
	_, err = app.StakingKeeper.BeginRedelegation(ctx, addrDels[1], addrVals[0], addrVals[1], sdk.NewDecFromInt(redelegateTokens), sdk.NewCoin(sdk.DefaultBondDenom, redelegateTokens))
	require.NoError(t, err)
...
func TestRedelegateFromUnbondedValidator(t *testing.T) {
	...
	_, err = app.StakingKeeper.BeginRedelegation(ctx, addrDels[1], addrVals[0], addrVals[1], sdk.NewDecFromInt(redelegationTokens), sdk.NewCoin(sdk.DefaultBondDenom, redelegationTokens))
	require.NoError(t, err)
