...
func (suite *KeeperTestSuite) TestGRPCQueryUnbondingDelegation() {
	...
	suite.NoError(err1)
	_, err := app.StakingKeeper.Undelegate(ctx, addrAcc2, valAddr, sdk.NewDecFromInt(unbondingTokens), sdk.NewCoin(sdk.DefaultBondDenom, unbondingTokens))
	suite.NoError(err)
...
func (suite *KeeperTestSuite) TestGRPCQueryDelegatorUnbondingDelegations() {
	...
	suite.NoError(err1)
	_, err := app.StakingKeeper.Undelegate(ctx, addrAcc, valAddr1, sdk.NewDecFromInt(unbondingTokens), sdk.NewCoin(sdk.DefaultBondDenom, unbondingTokens))
	suite.NoError(err)
	valAddr2, err1 := sdk.ValAddressFromBech32(addrVal2)
	suite.NoError(err1)
	_, err = app.StakingKeeper.Undelegate(ctx, addrAcc, valAddr2, sdk.NewDecFromInt(unbondingTokens), sdk.NewCoin(sdk.DefaultBondDenom, unbondingTokens))
	suite.NoError(err)
...
func (suite *KeeperTestSuite) TestGRPCQueryRedelegations() {
	...
	_, err = app.StakingKeeper.BeginRedelegation(ctx, addrAcc1, val1.GetOperator(), val2.GetOperator(), sdk.NewDecFromInt(rdAmount), sdk.NewCoin(sdk.DefaultBondDenom, rdAmount))
	suite.NoError(err)
	applyValidatorSetUpdates(suite.T(), ctx, app.StakingKeeper, -1)
...
func createValidators(t *testing.T, ctx sdk.Context, app *simapp.SimApp, powers []int64) ([]sdk.AccAddress, []sdk.ValAddress, []types.Validator) {
	...
	_, err := app.StakingKeeper.Delegate(ctx, addrs[0], sdk.NewCoin(sdk.DefaultBondDenom, app.StakingKeeper.TokensFromConsensusPower(ctx, powers[0])), types.Unbonded, val1, true)
	require.NoError(t, err)
	_, err = app.StakingKeeper.Delegate(ctx, addrs[1], sdk.NewCoin(sdk.DefaultBondDenom, app.StakingKeeper.TokensFromConsensusPower(ctx, powers[1])), types.Unbonded, val2, true)
	require.NoError(t, err)
	_, err = app.StakingKeeper.Delegate(ctx, addrs[0], sdk.NewCoin(sdk.DefaultBondDenom, app.StakingKeeper.TokensFromConsensusPower(ctx, powers[2])), types.Unbonded, val2, true)
	require.NoError(t, err)
